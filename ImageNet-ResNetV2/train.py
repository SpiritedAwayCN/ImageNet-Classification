...
if __name__=='__main__':
    ...
    check_point_files = [int(re.findall(r"(\d+).h5",file)[0]) for file in os.listdir("./") if re.match(r'resnetV2-18-\d+.h5$', file)]
    ...
    if init_epoch==0 and not os.path.isfile("resnetV2-18-warmup.h5"):
        # warm up - 1 epoch
        warmup(model, train_iter)
        model.save_weights('resnetV2-18-warmup.h5')
        test(model)
    elif init_epoch==0:
        model.load_weights('resnetV2-18-warmup.h5')
    else:
        model.load_weights('resnetV2-18-{:0>2}.h5'.format(init_epoch))
    ...
    for epoch in range(init_epoch, c.total_epoches):
        ...
        model.save_weights('resnetV2-18-{:0>2}.h5'.format(epoch + 1))
        test(model)