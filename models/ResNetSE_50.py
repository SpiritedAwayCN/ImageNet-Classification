...
class SELayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        res = self.global_average_pooling(inputs)
        res = self.fc_1(res)
        res = self.fc_2(res)

        outputs = tf.reshape(res,[-1,1,1,self.channel])
        return inputs * outputs

class SEBottleneckBlock(tf.keras.layers.Layer):
...
class ResNetSE_50(tf.keras.models.Model):
    def __init__(self, **kwargs):
        ...
        for i in range(1, 5):
            if i == 1:
                self.block_collector.append(SEBottleneckBlock(filters_num[i-1], projection=True, name='conv1_0'))
            else:
                self.block_collector.append(SEBottleneckBlock(filters_num[i-1], strides=(2, 2), name='conv{}_0'.format(i)))

            for j in range(1, block_num[i-1]):
                self.block_collector.append(SEBottleneckBlock(filters_num[i-1], name='conv{}_{}'.format(i, j)))
