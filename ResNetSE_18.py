...
class ResNetSE_18(tf.keras.models.Model):
    def __init__(self, **kwargs):
        super(ResNetSE_18, self).__init__(**kwargs)
        ...
        for i in range(1, 5):
            if i == 1:
                self.block_collector.append(SEBasicBlock(filters_num[i-1], name='conv1_0'))
            else:
                self.block_collector.append(SEBasicBlock(filters_num[i-1], strides=(2, 2), name='conv{}_0'.format(i)))

            for j in range(1, block_num[i-1]):
                self.block_collector.append(SEBasicBlock(filters_num[i-1], name='conv{}_{}'.format(i, j)))
